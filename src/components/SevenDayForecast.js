...
const SevenDayForecast = ({weatherIcons}) => {
    ...
    return (  
        <div className="self-stretch row-span-2 bg-light-obsidian w-400 grid gric-rows-8 p-20 rounded-xl ">
            ...
            {error && <p>{error}</p> }
            {days && days.map((day) => ( 
                <div className="self-center justify-self-center grid grid-cols-3 justify-center items-center w-350 h-80 border-b-2 last:border-b-0 mt-10 pb-10 duration-300 hover:scale-105 cursor-pointer">
