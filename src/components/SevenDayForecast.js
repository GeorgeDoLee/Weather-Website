...
const SevenDayForecast = ({weatherIcons, units}) => {
    ...
    return (
        <div className="self-stretch row-span-2 bg-light-obsidian w-400 grid grid-rows-forecast p-[20px] rounded-xl bg-opacity-50">
            ...
            <div className="justify-self-center self-start grid grid-rows-8">
                {daily && daily.map((day, index) => (
                    <div key={index} className={`self-center justify-self-center grid grid-cols-3 items-center w-[350px] h-[75px] border-b-2 last:border-b-0 mt-2 pb-2 duration-300 hover:scale-105 cursor-pointer ${animationIndex === index ? 'scale-105': ''}`}>
                        <h5 className="justify-self-start">{day.day}</h5>
                        <img src={weatherIcons[day.id]} alt="icon not found" title={day.weather} className="justify-self-center h-[50px]"/>
